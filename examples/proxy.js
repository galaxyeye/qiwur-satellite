...
fs.watchFile(config.black_list, function(c, p) {
	update_blacklist();
});

fs.watchFile(config.allow_ip_list, function(c, p) {
	update_iplist();
});

fs.watchFile(config.host_filters, function(c, p) {
	update_hostfilters();
});
...
function update_iplist() {
	update_list("Updating allowed ip list.", config.allow_ip_list,
			function(ip) {
				return ip;
			}, function(list) {
				iplist = list;
...
function action_proxy(response, request, host) {
	...
	// detect HTTP version
	var legacy_http = request.httpVersionMajor == 1 
		&& request.httpVersionMinor < 1 || request.httpVersionMajor < 1;
	...
	proxy.on('error', function(err) {
		...
		return action_notfound(response, "Requested resource (" + request.url
				+ ") is not accessible on host \"" + host + "\"");
	});
	...
	proxy_request.addListener('response', function(proxy_response) {
		if (legacy_http && proxy_response.headers['transfer-encoding'] != undefined) {
			...
			proxy_response.addListener('end', function() {
				// cancel transfer encoding "chunked"
...
function security_filter(request, response) {
	...
	if (request.headers.host === undefined || request.method === undefined
			|| request.url === undefined) {
		security_log(request, response,
				"Either host, method or url is poorly defined");
		return false;
	}
...
function server_cb(request, response) {
	...
	if (!security_filter(request, response))
		return;
...
config.listen_ssl.forEach(function(listen) {
	util.log("Starting *secure* reverse proxy server on port '" + listen.ip
			+ ':' + listen.port);
	var options = {
		cert : listen.cert,
		key : listen.key,
